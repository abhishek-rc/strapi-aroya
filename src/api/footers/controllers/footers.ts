 ...
 * footers controller
 * 
 * Extends the core footer controller to include navigation data
 * in the response. Fetches navigation data and combines it with footer
 * content in a single API call with locale support.
 */

import { factories } from '@strapi/strapi';

const NAVIGATION_SLUG = 'footer-navigation';
...
export default factories.createCoreController('api::footers.footers', ({ strapi }) => ({
    async find(ctx) {
        const locale = ctx.query.locale || null;
        const { data, meta } = await super.find(ctx);
        const footerNavigation = await fetchNavigation(strapi, NAVIGATION_SLUG, locale);

        return {
            data: Array.isArray(data)
                ? data.map((item) => ({ ...item, footer_navigation: footerNavigation }))
                : { ...data, footer_navigation: footerNavigation },
            meta,
        };
    },
    ...
    async findOne(ctx) {
        ...
        return {
            data: { ...data, footer_navigation: footerNavigation },
            meta,
        };
    },
}));
